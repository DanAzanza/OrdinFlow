...
let pendingSelectSkillFolder = null;

function openSelectSkillModal(folder) {
	pendingSelectSkillFolder = folder;
	const label = document.getElementById("selectSkillFolderLabel");
	if (label) label.textContent = folder;

	const dropdown = document.getElementById("selectSkillDropdown");
	if (dropdown) {
		const exportSkills = (state.skills || []).filter((s) => s.type !== "import");
		dropdown.innerHTML = exportSkills
			.map(
				(s) =>
		...
		if (exportSkills.length === 0) {
			dropdown.innerHTML = '<option value="">No export skills defined</option>';
		}
	}

	const modal = document.getElementById("selectSkillModal");
	if (modal) modal.classList.add("open");
}

function closeSelectSkillModal() {
	const modal = document.getElementById("selectSkillModal");
	if (modal) modal.classList.remove("open");
	pendingSelectSkillFolder = null;
}

async function confirmApproveAndRun() {
	if (!pendingSelectSkillFolder) return;
	const folder = pendingSelectSkillFolder;
	const dropdown = document.getElementById("selectSkillDropdown");
	const selectedSkillId = dropdown ? dropdown.value : null;

	try {
		// 1. Approve folder
		await api("/api/cases/approve", {
			method: "POST",
			body: JSON.stringify({ folder: folder, approved: true }),
		});

		// 2. If skill selected, enqueue it
		if (selectedSkillId) {
			await api("/api/skills/queue/add", {
				method: "POST",
				body: JSON.stringify({
					skill_id: selectedSkillId,
					context: { folder: folder, case_folder: folder },
				}),
			});
		}

		toast("🚀 Case approved & skill queued!", "success");
		closeSelectSkillModal();
		await fetchCases();
	} catch (e) {
		toast("Error approving & running skill: " + e.message, "error");
	}
}
